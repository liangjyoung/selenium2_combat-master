   ...
   // public static Logger logger = Logger.getLogger(SeleniumUtil.class.getName());
    ...
    public void launchBrowser(String browserName, ITestContext context,String webUrl,int timeOut) {
        ...
        try {
            ...
            FileHelper.info("注意：页面没有完全加载出来，刷新重试！！");
            refresh();
    ...
    public void maxWindow(String browserName) {
        ...
        FileHelper.info("最大化浏览器:" + browserName);
    ...
    public void submit(WebElement w){
        ...
        w.submit();
            ...
            FileHelper.info("在元素："+w+"做的提交操作失败");
            Assert.fail("在元素："+w+"做的提交操作失败");
        }
        //logger.info("在元素："+w+"做了提交操作");
        FileHelper.info("在元素："+w+"做了提交操作");
    }
    ...
    public void click(WebElement element) {
        ...
        try {
            ...
            FileHelper.info("The element you clicked:[" + element.toString() + "] is no longer exist!");
           ...
           // logger.error("Failed to click element [" + element.toString() + "]");
            FileHelper.info("Failed to click element [" + element.toString() + "]");
            Assert.fail("Failed to click element [" + element.toString() + "]",e);
        }
       // logger.info("点击元素 [" + element.toString() + "]");
        FileHelper.info("点击元素 [" + element.toString() + "]");
    }
    ...
    public void clickTheClickable(By byElement, long startTime, int timeOut) throws Exception {
        try {
            ...
            if (System.currentTimeMillis() - startTime > timeOut) {
                ...
                FileHelper.info(byElement+ " is unclickable");
                throw new Exception(e);
            } else {
                Thread.sleep(500);
               // logger.warn(byElement + " is unclickable, try again");
                FileHelper.info(byElement + " is unclickable, try again");
    ...
    public void clear(By byElement) {
        try {
            ...
            FileHelper.info("清除元素 [" + byElement + "] 上的内容失败!");
        }
       // logger.info("清除元素 [" + byElement  + "]上的内容");
        FileHelper.info("清除元素 [" + byElement  + "]上的内容");
    }
    ...
    public void type(By byElement, String key) {
        try {
            ...
            FileHelper.info("输入 [" + key + "] 到 元素[" + byElement + "]失败");
            Assert.fail("输入 [" + key + "] 到 元素[" + byElement + "]失败",e);
        }
        //logger.info("输入：[" + key + "] 到 [" + byElement + "]");
        FileHelper.info("输入：[" + key + "] 到 [" + byElement + "]");
    }
    ...
    public void waitForElementToLoad(int timeOut, final By By) {
        ...
        try {
           ...
           // logger.error("超时!! " + timeOut + " 秒之后还没找到元素 [" + By + "]");
        	FileHelper.info("超时!! " + timeOut + " 秒之后还没找到元素 [" + By + "]");
            Assert.fail("超时!! " + timeOut + " 秒之后还没找到元素 [" + By + "]");

        }
       // logger.info("找到了元素 [" + By + "]");
        FileHelper.info("找到了元素 [" + By + "]");
    }
     ...
     int find=0;
        ...
        while(flag){
            if(notFindTimes==3){
               ...
               // logger.error("尝试了3次查找都未查找到元素："+By+"请检查是不是网络或者网站性能问题（响应速度不够）");
            	FileHelper.info("尝试了3次查找都未查找到元素："+By+"请检查是不是网络或者网站性能问题（响应速度不够）");
                Assert.fail("尝试了3次查找都未查找到元素："+By+"请检查是不是网络或者网站性能问题（响应速度不够）");
            }
        //logger.info("开始第"+(notFindTimes+1)+"次查找元素[" + By + "]");
            FileHelper.info("开始第"+(notFindTimes+1)+"次查找元素[" + By + "]");
        try {
            (new WebDriverWait(driver, timeOut)).until(new ExpectedCondition<Boolean>() {
           ...
           // logger.warn("超时!! " + timeOut + " 秒之后还没找到元素 [" + By + "],这是第"+(notFindTimes+1)+"次查找！");
        	FileHelper.info("超时!! " + timeOut + " 秒之后还没找到元素 [" + By + "],这是第"+(notFindTimes+1)+"次查找！");
            notFindTimes++;
            if(notFindTimes<3){
            refresh();
            }
    ...
    public void isTextCorrect(String actual, String expected) {
        try {
           ...
           // logger.error("期望的文字是 [" + expected + "] 但是找到了 [" + actual + "]");
        	FileHelper.info("期望的文字是 [" + expected + "] 但是找到了 [" + actual + "]");
            Assert.fail("期望的文字是 [" + expected + "] 但是找到了 [" + actual + "]");

        }
       // logger.info("找到了期望的文字: [" + expected + "]");
        FileHelper.info("找到了期望的文字: [" + expected + "]");
    ...
    public void pause(int sleepTime) {
        ...
        try {
           ...
           // logger.info("暂停:"+sleepTime+"秒");
            FileHelper.info("暂停:"+sleepTime+"秒");
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    ...
    public boolean doesCheckboxSelected(By elementLocator) {
        if (findElementBy(elementLocator).isSelected() == true) {
           // logger.info("CheckBox: " + getLocatorByElement(findElementBy(elementLocator), ">") + " 被勾选");
        	FileHelper.info("CheckBox: " + getLocatorByElement(findElementBy(elementLocator), ">") + " 被勾选");
            return true;
        } else
           // logger.info("CheckBox: " + getLocatorByElement(findElementBy(elementLocator), ">") + " 没有被勾选");
        	FileHelper.info("CheckBox: " + getLocatorByElement(findElementBy(elementLocator), ">") + " 没有被勾选");
        return false;
    ...
    public void executeJS(String js) {
       ...
       // logger.info("执行JavaScript语句：[" + js + "]");
        FileHelper.info("执行JavaScript语句：[" + js + "]");
    }
    ...
    public String getInputValue(String chose,String choseValue) {
        ...
        case "id":
            ...
            default:
            	...
            	FileHelper.info("未定义的chose:"+chose);
    ...
    public String getLocatorByElement(WebElement element, String expectText) {
        ...
        try {
           ...
           // logger.error("failed to find the string [" + expectText + "]");
            FileHelper.info("failed to find the string [" + expectText + "]");
    ...
    public boolean isDisplayed(WebElement element) {
        ...
        if (element.isDisplayed()) {
        	...
        	FileHelper.info("The element: [" + getLocatorByElement(element, ">") + "] is displayed");
            isDisplay = true;
        } else if (element.isDisplayed() == false) {
           // logger.warn("The element: [" + getLocatorByElement(element, ">") + "] is not displayed");
        	FileHelper.info("The element: [" + getLocatorByElement(element, ">") + "] is not displayed");
            isDisplay = false;
        }
        return isDisplay;
    }
    ...
    public boolean isSelected(WebElement element) {
        ...
        if (element.isSelected() == true) {
           // logger.info("The element: [" + getLocatorByElement(element, ">") + "] is selected");
        	...
        	FileHelper.info("The element: [" + getLocatorByElement(element, ">") + "] is not selected");
            flag = false;
        }
        return flag;
    }
    ...
    public void isContains(String actual, String expect) {
        try {
        	...
        	FileHelper.info("The [" + actual + "] is not contains [" + expect + "]");
    ...
    public void stopLoad(){    
        ...
        try {
           ...
           // logger.info("按下了Esc键");
            FileHelper.info("按下了Esc键");
            r.keyRelease(KeyEvent.VK_ESCAPE);
            //logger.info("松开了Esc键");
            FileHelper.info("松开了Esc键");
        } catch (AWTException e) {
            e.printStackTrace();
        }

       // logger.info("正在停止页面加载...");
        FileHelper.info("正在停止页面加载...");
    }
    ...
    public boolean isAlertPresent(){
        try
        {
            ...
            //logger.info("alert出现");
            FileHelper.info("alert出现");
            return true;
        }   
        catch (NoAlertPresentException Ex)
        {
           // logger.warn("alert没有出现");
        	FileHelper.info("alert没有出现");
            return false;
        }   
}
